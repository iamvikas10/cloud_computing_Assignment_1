...
public class RatingMapperMR extends Mapper <Object, Text, Text, Text>{

	private String table1 = null;
	private String attributeIndex1 = null;
	private String val1 = null;
	
	public static final Log log = LogFactory.getLog(RatingMapperMR.class);
...
//			log.info("val UsersMapper:" + val1);
			...
			if(table1!=null)
			{
				...
				if(str.equals(val1))
				{
					context.write(new Text(parts[1]), new Text("Rating," + parts[0] +","+ parts[1] +","+ parts[2] +","+ parts[3]));
				}
			}
			else
				context.write(new Text(parts[1]), new Text("Rating," + parts[0] +","+ parts[1] +","+ parts[2] +","+ parts[3]));
