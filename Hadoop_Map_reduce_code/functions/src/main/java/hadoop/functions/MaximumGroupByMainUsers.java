...
public class MaximumGroupByMainUsers extends Configured implements Tool {
    @Override
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf,"MaxGroupByMain");
        job.setJobName("MaxGroupBy");
        job.setJarByClass(MaximumGroupByMainUsers.class);

        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(Text.class);

        job.setMapperClass(MaxMapUsers.class);
        job.setCombinerClass(MaxReduceUsers.class);
        job.setReducerClass(MaxReduceUsers.class);
    ...
    public static void main(String[] args) throws Exception{
        int exitCode = ToolRunner.run(new MaximumGroupByMainUsers(), args);
