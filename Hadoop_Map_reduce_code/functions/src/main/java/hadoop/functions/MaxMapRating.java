...
public class MaxMapRating extends Mapper<LongWritable, Text, Text, Text> {
...
//        groupByColumn.set(row[x]);
        ...
        maxValue = Integer.parseInt(row[Integer.parseInt(funColumnConf)]);
        ...
        if(len == 5){
            context.write(groupByColumn, new Text(row[arr[0]] + ","+ row[arr[1]] + "," + row[arr[2]] + ","+ row[arr[3]] +","+row[arr[4]]+ ","+ maxValue));
        }
        else if(len ==4){
            context.write(groupByColumn, new Text(row[arr[0]] + ","+ row[arr[1]] + "," + row[arr[2]] + ","+ row[arr[3]] +","+ maxValue));
        }
        else if(len == 3){
            context.write(groupByColumn, new Text(row[arr[0]] + ","+ row[arr[1]] + "," + row[arr[2]]  +","+ maxValue));
        }
        else if(len == 2){
            context.write(groupByColumn, new Text(row[arr[0]] + ","+ row[arr[1]] + "," + maxValue));
            //System.out.println("just Testing");
        }
        else if(len == 1){
            context.write(groupByColumn, new Text(row[arr[0]] + ","+  maxValue));
        }
    }
}
