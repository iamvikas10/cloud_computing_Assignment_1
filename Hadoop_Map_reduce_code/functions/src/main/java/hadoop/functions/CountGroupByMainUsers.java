...
public class CountGroupByMainUsers extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "CountGroupByDynamic");
        job.setJobName("CountGroupByDyanmic");
        job.setJarByClass(CountGroupByMainUsers.class);

        job.setOutputKeyClass(Text.class);
        job.setOutputValueClass(IntWritable.class);

        job.setMapperClass(CountMapUsers.class);
        job.setCombinerClass(CountReduceUsers.class);
        job.setReducerClass(CountReduceUsers.class);
    ...
    public static void main(String args[]) throws Exception{
        int exitCode = ToolRunner.run(new CountGroupByMainUsers(), args);
